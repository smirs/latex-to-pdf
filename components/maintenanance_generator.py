...
MAIN_TEX_FILE = "addundum_maintenance.tex"

def generate_maintenance_pdf(form_data):
    ...
    placeholders = {
        ...
        "{{PENALTYFINE}}": form_data.get("penaltyfine", ""),
    }

    for k, v in placeholders.items():
        latex_content = latex_content.replace(k, v)

    tex_filename = "output_maintenance.tex"
    tex_path = os.path.join(TEX_DIR, tex_filename)
    with open(tex_path, "w") as f:
        f.write(latex_content)

    os.makedirs(OUTPUT_FOLDER, exist_ok=True)
    pdf_path = os.path.join(OUTPUT_FOLDER, "output_maintenance.pdf")
